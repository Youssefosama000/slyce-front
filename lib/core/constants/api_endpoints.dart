...
class ApiEndpoints {
  ...
  // Read the saved customer profile (gender, weight, height, activity rate,
  // allergen ids, diet-preference ids). Allergen/diet display names are
  // resolved via foodAllergens / foodPreferences below.
  static const String customerProfileGet = '/$apiVersion/customers/profile';
  ...
  // Body: { mealId, sizeId, quantity }. (Slyce collection → "Change item qty".)
  static const String cartItemQuantity = '/$apiVersion/cart/items/quantity';
  // Remove a specific line from the cart. Body: { mealId, sizeId }.
  // (Slyce collection → DELETE /cart/items, returns 204 No Content.)
  ...
  // Search meals near a location: GET /v1/meals/nearby?searchTerm=&lat=&lng=
  static const String mealsNearby = '/$apiVersion/meals/nearby';
