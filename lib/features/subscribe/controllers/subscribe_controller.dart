...
class SubscribeController extends GetxController {
  ...
  // ── Branch availability (days + time slots) ─────────────────────────

  /// Hour ranges (24h) for each delivery time slot, compared against a
  /// branch's working hours. Values are [startHour, endHour]; 24 == midnight.
  static const Map<String, List<int>> timeSlotHours = {
    'EarlyMorning': [6, 9],
    'Morning': [9, 12],
    'Afternoon': [12, 15],
    'LateAfternoon': [15, 18],
  ...
  static const Set<String> _allDays = {
    'Sunday',
    'Monday',
    'Tuesday',
    'Wednesday',
  ...
  /// The branch currently driving availability (the selected one, else the
  /// first available).
  ...
  /// Day names the selected branch is open on. When the branch exposes no
  /// working hours, every day is treated as available (so nothing is locked
  /// without data).
  Set<String> get availableDays {
    final branch = _selectedBranch;
    if (branch == null || branch.openingHours.isEmpty) return _allDays;
    final result = <String>{};
    for (final h in branch.openingHours) {
      final name = _normalizeDay(h.day);
      if (name != null) result.add(name);
    }
    return result.isEmpty ? _allDays : result;
  }

  bool isDayAvailable(String fullDay) => availableDays.contains(fullDay);

  /// Whether [slot] overlaps any of the branch's working-hour ranges. Unknown
  /// or missing hours => the slot is available.
  bool isTimeSlotAvailable(String slot) {
    final branch = _selectedBranch;
    if (branch == null || branch.openingHours.isEmpty) return true;
    final range = timeSlotHours[slot];
    if (range == null) return true;
    final slotStart = range[0].toDouble();
    final slotEnd = range[1].toDouble();
    var sawValidHours = false;
    for (final h in branch.openingHours) {
      final open = _parseHour(h.open);
      var close = _parseHour(h.close);
      if (open == null || close == null) continue;
      sawValidHours = true;
      if (close <= 0) close = 24; // midnight close
      if (slotStart < close && open < slotEnd) return true;
    }
    // If no entry had parseable times, don't lock anything.
    return sawValidHours ? false : true;
  }

  /// Drop any selected day / time slot the (new) branch can't fulfil.
  void _pruneSelectionsForBranch() {
    final avail = availableDays;
    selectedDays.removeWhere((d) => !avail.contains(d));
    if (!isTimeSlotAvailable(selectedTimeSlot.value)) {
      selectedTimeSlot.value = timeSlots.firstWhere(
        isTimeSlotAvailable,
        orElse: () => selectedTimeSlot.value,
      );
    }
  }
  ...
  static String? _normalizeDay(String raw) {
    ...
    for (final entry in map.entries) {
      if (entry.key.startsWith(r) ||
          r.startsWith(entry.key.substring(0, 3))) {
        return entry.value;
      }
    }
    return null;
  ...
  Future<void> loadMealsForRestaurant(Restaurant restaurant) async {
    ...
    if (restaurant.menu.isNotEmpty) {
      await _setMealsFiltered(restaurant.menu);
      return;
    }
  ...
  /// Assign meals to the subscription picker after dropping any that no longer
  /// exist in the live catalogue. Cached menu feeds can still list meals that
  /// were deleted on the backend; this hides them here the same way the home
  /// feed and restaurant page do.
  Future<void> _setMealsFiltered(List<MenuItem> items) async {
    final live = await _filterToLiveMeals(items);
    meals.assignAll(live);
  }
  ...
  /// meal is kept, so a transient failure never wrongly empties the picker.
  ...
  Future<List<MenuItem>> _filterToLiveMeals(List<MenuItem> items) async {
    ...
    final checks = await Future.wait(
      items.map((item) async {
        ...
        try {
          await _menuRepo.getMealById(item.id);
          return item;
        } on ApiException catch (e) {
  ...
  void _applyHome(HomeController home) {
    ...
    if (hasPreloaded && current == preferredId && meals.isEmpty) {
      _setMealsFiltered(home.allMeals);
    }
  }
  ...
  void selectRestaurant(String restaurantId, {List<MenuItem>? preloadedMeals}) {
    ...
    if (preloadedMeals != null && preloadedMeals.isNotEmpty) {
      _setMealsFiltered(preloadedMeals);
    } else {
      loadMealsForRestaurant(restaurant);
    }
  }
