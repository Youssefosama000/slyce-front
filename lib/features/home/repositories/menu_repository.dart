...
class MenuRepository {
  ...
  /// Search meals near a location (GET /v1/meals/nearby).
  ///
  /// Returns lightweight meal hits (id, name, image, calories, price) so the
  /// in-app search reflects the full nearby catalogue, not just the already
  /// loaded home feed. The response shape is `{ "meals": [ ... ] }`.
  Future<List<MealModel>> searchMealsNearby({
    required String term,
    required double latitude,
    required double longitude,
  }) async {
    final response = await _client.get(
      ApiEndpoints.mealsNearby,
      queryParameters: {
        'searchTerm': term,
        'lat': latitude,
        'lng': longitude,
